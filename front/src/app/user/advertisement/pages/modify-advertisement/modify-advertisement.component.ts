...
export class ModifyAdvertisementComponent implements OnInit {
  ...
  constructor(
    ...
    this.adForm = this.fb.group({
      ...
      photoUrls: [''],
      active: [true] // Campo para activar/desactivar el anuncio
    });
  }
    ...
    this.http.get<any>(`/public/advertisements/${this.advertisementId}`, { headers }).subscribe({
      next: (data) => {
        ...
        this.adForm.patchValue({
          ...
          active: data.active // Cargar el estado actual del anuncio
        });
  ...
  onSubmit(): void {
    if (this.adForm.valid) {
      ...
      const requestBody = {
        ...
        active: this.adForm.value.active // Enviar el estado activo/desactivo
      };
